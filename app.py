...
Using psycopg (v3) - no psycopg2 dependency
"""
    ...
    with get_db() as conn:
        try:
            ...
            for row in rows:
                ...
                stories_dict[uid]['stories'].append({
                    'id': story['id'],
    ...
    with get_db() as conn:
        try:
            existing = conn.execute(
                "SELECT id FROM likes WHERE user_id = %s AND post_id = %s", (user_id, post_id)
            ).fetchone()

            if existing:
                conn.execute("DELETE FROM likes WHERE user_id = %s AND post_id = %s", (user_id, post_id))
                liked = False
            else:
                conn.execute("INSERT INTO likes (user_id, post_id) VALUES (%s, %s)", (user_id, post_id))
                post_owner = conn.execute("SELECT user_id FROM posts WHERE id = %s", (post_id,)).fetchone()
...
def toggle_follow():
    ...
    with get_db() as conn:
        try:
            ...
            if existing:
                conn.execute("DELETE FROM follows WHERE follower_id = %s AND following_id = %s", (follower_id, following_id))
                followed = False
            else:
                conn.execute("INSERT INTO follows (follower_id, following_id) VALUES (%s, %s)", (follower_id, following_id))
                conn.execute(
    ...
    with get_db() as conn:
        try:
            rows = conn.execute("""
                ...
                LIMIT 20
            """, (user_id, f'%{query}%', user_id)).fetchall()
...
def get_profile():
    ...
    with get_db() as conn:
        try:
            user = conn.execute(
                "SELECT id, username, bio, profile_pic, created_at FROM users WHERE id = %s", (user_id,)
            ).fetchone()
            if not user:
            ...
            followers_count = conn.execute(
                "SELECT COUNT(*) as count FROM follows WHERE following_id = %s", (user_id,)
            ).fetchone()['count']

            following_count = conn.execute(
                "SELECT COUNT(*) as count FROM follows WHERE follower_id = %s", (user_id,)
            ).fetchone()['count']
    ...
    with get_db() as conn:
        try:
            ...
            unread_count = conn.execute(
                "SELECT COUNT(*) as count FROM notifications WHERE user_id = %s AND read = FALSE", (user_id,)
            ).fetchone()['count']
